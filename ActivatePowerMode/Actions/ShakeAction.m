...
+ (void)shakeView:(NSView *)view
{
    ...
    /** Modified by Frank http://github.com/wolfcon */
    
    // 来, 摇动上层View, 摇动的更自由一些吧! (free the textView, rock from all directions!)
    NSView *letsRockSuperView = view.superview.superview;
    ...
    [self moveView:letsRockSuperView toPoint:CGPointMake(x, y)];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(shakeDurationTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self moveView:letsRockSuperView toPoint:CGPointMake(0, 0)];
    });
}
